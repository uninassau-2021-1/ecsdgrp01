...
router.post('/', function (requisicao, resposta, next) {
  ...
  if (
    !(dados.usuario.charCodeAt(0) >= 97 && dados.usuario.charCodeAt(0) <= 122)
  ) {
  ...
  for (let index = 0; index < dados.usuario.length; index++) {
    if (
      (dados.usuario.charCodeAt(index) >= 48 &&
        dados.usuario.charCodeAt(index) <= 57) ||
      (dados.usuario.charCodeAt(index) >= 97 &&
        dados.usuario.charCodeAt(index) <= 122)
    ) {
      i++;
    }
  }
  ...
  for (let index = 0; index < dados.senha.length; index++) {
    if (
      (dados.senha.charCodeAt(index) >= 48 &&
        dados.senha.charCodeAt(index) <= 57) ||
      (dados.senha.charCodeAt(index) >= 65 &&
        dados.senha.charCodeAt(index) <= 90) ||
      (dados.senha.charCodeAt(index) >= 97 &&
        dados.senha.charCodeAt(index) <= 122)
    ) {
      i++;
    }
  }
  if (i != dados.senha.length) {
    erros.push(
      "A senha só pode ter números, letras minúsculas ou letras maiúsculas."
    );
  }
...
router.post("/bloquear_usuario", function (requisicao, resposta, next) {
  ...
  banco.query(
    `UPDATE usuarios SET usuario_bloqueado = 1 WHERE usuario = '${usuario}'`,
    function (err, result) {
